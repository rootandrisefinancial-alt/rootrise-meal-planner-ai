...
Only include the days requested. Only include meal slots that are NOT already locked.`;
  ...
  const makeRequest = async () => {
    ...
    if (!response.ok) {
      ...
      if (response.status === 529 || errData?.error?.type === "overloaded_error") {
  ...
  // Retry up to 3 times with delay on overload
...
// ─── Meal Plan Tab ────────────────────────────────────────────────
function MealPlanTab({ meals, setMeals, inventory, preferences, setPreferences, groceryList, setGroceryList }) {
  ...
  const viewRecipe = async (mealName) => {
    ...
    try {
      ...
      setRecipeError(e.message === "overloaded"
        ? "AI is a little busy right now - please try again in a moment!"
        : "Couldn't load recipe - please try again.");
    }
  ...
  return (
    <div>
                ...
                return (
                  <div key={meal} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    ...
                    <div style={{ flex: 1, position: "relative", display: "flex", alignItems: "center" }}>
                      ...
                      {mealVal && (
                        <button
                          onClick={() => !loadingRecipe && !recipeLimitReached && viewRecipe(mealVal)}
                          style={{
                            position: "absolute", right: "6px",
                            background: recipeLimitReached ? COLORS.muted : loadingRecipe === mealVal ? COLORS.gold : COLORS.sage,
                            color: COLORS.white, border: "none", borderRadius: "6px",
                            fontSize: "10px", fontFamily: "Arial, sans-serif", fontWeight: "700",
                            padding: "4px 8px", cursor: (loadingRecipe || recipeLimitReached) ? "default" : "pointer",
                            opacity: recipeLimitReached ? 0.5 : 1, whiteSpace: "nowrap",
                            transition: "background 0.2s",
                          }}
                        >
                          {loadingRecipe === mealVal ? "Loading..." : recipeLimitReached ? "Limit" : "Recipe"}
                        </button>
                      )}
                    </div>
                  </div>
      ...
      {activeRecipe && (
        ...
        }} onClick={() => { setActiveRecipe(null); setAddedIngredients({}); }}>
          <div
            style={{
            ...
            {activeRecipe.needToBuy?.length > 0 && (
              <div style={{ ...styles.card, background: `${COLORS.gold}12`, border: `1px solid ${COLORS.gold}40`, marginBottom: "12px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                  <div style={{ ...styles.label, color: COLORS.gold }}>You Will Need to Buy</div>
                  <button
                    onClick={() => {
                      const existingNames = new Set((groceryList || []).map(i => (typeof i === "string" ? i : i.name).toLowerCase()));
                      const newItems = activeRecipe.needToBuy
                        .map(ing => {
                          const name = ing.split(",")[0].trim();
                          return name;
                        })
                        .filter(name => !existingNames.has(name.toLowerCase()))
                        .map(name => ({ id: `item-${Date.now()}-${Math.random()}`, name, category: "misc" }));
                      if (newItems.length > 0) {
                        setGroceryList(prev => [...(prev || []), ...newItems]);
                      }
                      const allAdded = {};
                      activeRecipe.needToBuy.forEach(ing => { allAdded[ing] = true; });
                      setAddedIngredients(allAdded);
                    }}
                    style={{ ...styles.btnGold, fontSize: "11px", padding: "5px 10px", opacity: Object.keys(addedIngredients).length === activeRecipe.needToBuy?.length ? 0.6 : 1 }}
                  >
                    {Object.keys(addedIngredients).length === activeRecipe.needToBuy?.length ? "All Added!" : "+ Add All to List"}
                  </button>
                </div>
                {activeRecipe.needToBuy.map((ing, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 0", borderBottom: i < activeRecipe.needToBuy.length - 1 ? `1px solid ${COLORS.gold}20` : "none" }}>
                    <div style={{ fontFamily: "Arial, sans-serif", fontSize: "13px", color: COLORS.text }}>
                      + {ing}
                    </div>
                    <button
                      onClick={() => {
                        const name = ing.split(",")[0].trim();
                        const exists = (groceryList || []).some(i => (typeof i === "string" ? i : i.name).toLowerCase() === name.toLowerCase());
                        if (!exists) {
                          setGroceryList(prev => [...(prev || []), { id: `item-${Date.now()}-${Math.random()}`, name, category: "misc" }]);
                        }
                        setAddedIngredients(prev => ({ ...prev, [ing]: true }));
                      }}
                      style={{
                        ...styles.btnOutline, fontSize: "10px", padding: "3px 8px",
                        borderColor: addedIngredients[ing] ? COLORS.sage : COLORS.gold,
                        color: addedIngredients[ing] ? COLORS.sage : COLORS.gold,
                        whiteSpace: "nowrap", transition: "all 0.2s",
                      }}
                    >
                      {addedIngredients[ing] ? "Added" : "+ Add"}
                    </button>
                  </div>
                ))}
              </div>
...
export default function App() {
  ...
  const screens = [
    ...
    },
    {
      ...
      component: <MealPlanTab meals={meals} setMeals={setMeals} inventory={inventory} preferences={preferences} setPreferences={setPreferences} groceryList={groceryList} setGroceryList={setGroceryList} />,
    },
    {
      icon: "🛒", label: "Grocery",
