                  ...
                  <button
                    onClick={() => {
                      ...
                      const newItems = activeRecipe.needToBuy
                        .map(ing => {
                          const name = ing.replace(/^\d+[\s\w]*?\s+/, "").split(",")[0].trim();
                          return name;
                        })
                        .filter(name => !existingNames.has(name.toLowerCase()))
                        .map(name => ({ id: `item-${Date.now()}-${name}`, name, category: "misc" }));
                      if (newItems.length > 0) {
                        setGroceryList(prev => [...(prev || []), ...newItems]);
                      }
                    }}
                    style={{ ...styles.btnGold, fontSize: "11px", padding: "5px 10px" }}
                  >
                    + Add All to List
                  </button>
                </div>
                  ...
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 0", borderBottom: i < activeRecipe.needToBuy.length - 1 ? `1px solid ${COLORS.gold}20` : "none" }}>
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (!exists) {
                          setGroceryList(prev => [...(prev || []), { id: `item-${Date.now()}-${name}`, name, category: "misc" }]);
                        }
                      }}
                      style={{ ...styles.btnOutline, fontSize: "10px", padding: "3px 8px", borderColor: COLORS.gold, color: COLORS.gold, whiteSpace: "nowrap" }}
                    >
                      + Add
                    </button>
                  </div>
